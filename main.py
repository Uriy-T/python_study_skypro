...
def main() -> None:
    ...
    available_states_lower = [
        status.lower() for status in available_states
    ]

    choose_source_data = int(input(
    ...
    if choose_source_data == 2:
        ...
        transaction_data = csv_reader('data/operations_main.csv')

    if choose_source_data == 3:
        print('Для обработки выбран XLSX-файл.')
        transaction_data = xlsx_reader('data/operations_main.xlsx')
    ...
    while True:
        if choose_status in available_states_lower:
            filter_by_state_data = filter_by_state(transaction_data,
                                                   state=choose_status)
            ...
            choose_status = input(
                'Введите статус, по которому необходимо выполнить фильтрацию.\n'
                f'Доступные для фильтрации статусы: {available_states}').lower()
    ...
    while True:
        if is_sort_by_date == 'да':
            choose_sort_order = input('Отсортировать по возрастанию или по убыванию?\n'
                                      'Ответ: ').lower()
            if choose_sort_order in 'по убыванию':
                sorted_dy_date_data = sort_by_date(filter_by_state_data, sort_order=True)
                break
            elif choose_sort_order in 'по возрастанию':
                sorted_dy_date_data = sort_by_date(filter_by_state_data, sort_order=False)
                break
            else:
    ...
    rub_operation_output = input('Выводить только рублевые транзакции? Да/Нет\n'
                                 'Ответ: ')
    while True:
        if rub_operation_output == 'да':
            filter_by_currency_data = list(filter_by_currency(sorted_dy_date_data, currency='RUB'))
            print(filter_by_currency_data)
            break
    ...
    filter_by_descr = input('Отфильтровать список транзакций по определенному'
    ...
    while True:
        if filter_by_descr == 'да':
            ...
            result_output_data = search_by_descr(transactions=filter_by_currency_data,
    ...
    if result_transaction_count > 0:
        print(f'Всего банковских операций в выборке: {result_transaction_count}\n')
        descriptions = transaction_descriptions(result_output_data)

        for transaction in result_output_data:

            print(f'{get_date(transaction['date'])} '
                  f'{next(descriptions)}')
            if 'Перевод' in transaction['description']:
                print(f'{mask_account_card(transaction['from'])}'
