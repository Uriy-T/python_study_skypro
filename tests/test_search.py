...
@pytest.mark.parametrize(param_packer(search_test_valid_data),
                         value_packer(search_test_valid_data))
def test_search_by_descr_positive(
        transaction_data: list[dict[str, Any]],
        search_string: str,
        expected_result: list[dict[str, Any]]
) -> None:
    found_transactions = search_by_descr(transaction_data,
                                         search_string)
    assert found_transactions == expected_result


@pytest.mark.parametrize(param_packer(search_test_invalid_data),
...
def test_search_by_descr_negative(
    ...
    with pytest.raises(exception_type) as exc_info:
        search_by_descr(transaction_data,
                        search_string)
